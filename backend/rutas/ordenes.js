...
router.post('/sync', async (req, res) => {
    try {
        const { numero_orden, estado_orden, subtotal, total, fecha_creacion } = req.body;
        ...
        const query = `
            INSERT INTO ordenes (numero_orden, estado_orden, subtotal, total, fecha_creacion)
            VALUES ($1, $2, $3, $4, $5)
            ON CONFLICT (numero_orden) DO NOTHING
            RETURNING id_orden
        `;

        const result = await db.query(query, [numero_orden, estado_orden || 'pendiente', subtotal, total, fecha_creacion]);
