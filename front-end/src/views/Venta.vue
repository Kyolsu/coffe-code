...
const confirmOptionsAndAddToCart = () => {
  ...
  if (isPromoContext.value) {
    if (selectedPromo.value?.tipo === 'descuento_fijo') {
      // Flujo rápido: Añadir producto con 100% descuento y saltar el modal de promos
      const promoGroupId = Date.now()
      cart.value.push({
        cartId: promoGroupId,
        productId: selectedProductForOptions.value!.id_producto,
        name: selectedProductForOptions.value!.nombre_producto,
        qty: 1,
        basePrice: selectedProductForOptions.value!.precio_base,
        modifiers: [...selectedModifiers],
        unitTotal: 0, // 100% descuento
        categoria: selectedProductForOptions.value!.categoria,
      ...
      playSound('added');
      displayToast(`${selectedProductForOptions.value!.nombre_producto} agregado gratis`, 'success');
...
const openPromoModal = async (promo: Promocion) => {
  ...
  if (promo.tipo === 'descuento_fijo') {
    // Si hay exactamente 1 paquete vinculado a la promo
    if (isDescFijoPaquete && promo.paquetes?.length === 1) {
      const paq = paquetes.value.find(p => p.id_paquete === promo.paquetes![0])
      if (paq) {
        togglePromoPaquet(paq)
        return // Se salta abrir el modal de promo
      }
    }
    // Si hay exactamente 1 producto vinculado a la promo
    if (promo.productos?.length === 1) {
      const prod = allProducts.value.find(p => p.id_producto === promo.productos![0])
      if (prod) {
        togglePromoProduct(prod.id_producto)
        return // Se salta abrir el modal de promo
      }
    }
  }
...
const openPromoProductOptions = async (product: Producto) => {
  ...
  if (todasOpciones.length > 0) {
    ...
    if (selectedPromo.value?.tipo === 'descuento_fijo') {
      ...
      playSound('added');
      displayToast(`${product.nombre_producto} agregado gratis`, 'success');
